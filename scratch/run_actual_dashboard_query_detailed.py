...
-- Print debug info
SELECT @ActiveSlotIdx AS ActiveSlotIdx, @TheoreticalActiveSlotIdx AS TheoreticalActiveSlotIdx;
...
SELECT DATEADD(hour, @UTCOffset, time) AS time_local, time AS time_utc, metric, value, slot_idx FROM (
...
WHERE slot_idx = 10
ORDER BY time ASC, sort_time_order ASC;
"""

cursor.execute(query)
# Read results by traversing sets
set_num = 1
while True:
    try:
        if cursor.description is not None:
            print(f"--- Result Set {set_num} ---")
