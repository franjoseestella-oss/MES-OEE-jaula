...
try:
    ...
    if old_case in query:
        query = query.replace(old_case, new_case)
        print("Successfully replaced value CASE logic.")
    else:
        # Let's try replacing with normalized whitespaces if needed, but let's check first.
        print("Warning: could not find exact case block, searching/matching...")
