...
def main():
    ...
    SELECT 
        ...
        COALESCE(
            ...
            CASE 
                WHEN m.bastidor = @ActiveBastidor THEN @ActiveStartDT 
                ELSE (SELECT MIN(timestamp) FROM dbo.mes_machine_events WHERE secuencia_id = m.secuencia AND state = 'SECUENCIA_INICIADA' AND timestamp >= @ShiftStartDT)
            END
        ),
        log.FECHA_HORA_FIN_SEC
    FROM #MappedSeqs m
    ...
    FROM #SeqsToSchedule s
    WHERE s.actual_start IS NOT NULL AND s.actual_end IS NULL AND s.bastidor != @ActiveBastidor;
    ...
    while True:
        try:
            if cursor.description is not None:
                ...
                for r in rows:
                    if r[2] is not None and 'Esperando' not in str(r[2]):
