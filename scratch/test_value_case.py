...
SELECT 
    ...
    s.planned_start,
    s.planned_end,
    @CurrentProgressTime AS current_progress,
    -- Debug the upper limit case logic:
    CASE 
        WHEN s.planned_start >= @CurrentProgressTime THEN s.planned_start
        ELSE
            CASE 
                ...
                ELSE s.planned_end
            END
    END AS upper_limit_val,
    -- Debug the final CASE value logic:
    CASE 
        WHEN ft.t >= CASE 
                        -- Rule: If a previous sequence is in progress, do not draw this sequence
                        WHEN @ActiveSlotIdx IS NOT NULL AND s.slot_idx > @ActiveSlotIdx AND s.actual_start IS NULL AND @ActiveSlotIdx >= @TheoreticalActiveSlotIdx THEN
                        ...
                        ELSE
                            CASE 
                                ...
                                ELSE
                                    ...
                                    END
                            END
                    END THEN 'THEN NULL (greater or equal upper limit)'
        WHEN s.actual_start IS NOT NULL AND ft.t >= s.actual_start AND (s.actual_end IS NULL OR ft.t < s.actual_end) THEN 'Active/Process'
        ELSE 'Esperando máquina'
    END AS final_case_value
FROM FilteredTimestamps ft
INNER JOIN #SeqsToSchedule s ON s.id = ft.id
WHERE s.secuencia = '0261'
ORDER BY ft.t ASC;
"""
