...
SELECT time, metric, value FROM (
    ...
    -- Sequence planned start with its state (capped at shift boundaries)
    SELECT 
        CASE 
    ...
    -- Sequence planned end to terminate the block (capped at shift boundaries)
    SELECT 
        CASE 
            WHEN planned_end < @ShiftStartDT THEN @ShiftStartDT 
            WHEN planned_end > @ShiftEndDT THEN @ShiftEndDT
            ELSE planned_end 
        END AS time,
        CONCAT(secuencia, CHAR(10), bastidor) AS metric,
        NULL AS value
