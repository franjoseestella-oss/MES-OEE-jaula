...
SET @ActiveDate = CONVERT(varchar(8), CAST('2026-06-12' AS DATE), 112);
...
DECLARE @DashboardDate DATE = CAST('2026-06-12' AS DATE);
...
try:
    ...
    print("--- 2026-06-12 QUERY RESULTS ---")
    print(f"{'Seq':<5} | {'Bastidor':<14} | {'Modelo':<8} | {'Inicio Plan':<11} | {'Fin Plan':<11} | {'Inicio Real':<11} | {'Fin Real':<11} | {'Desviacion':<10} | {'Estado':<10}")
    print("-" * 120)
    for r in rows:
        print(f"{str(r[0]):<5} | {str(r[1]):<14} | {str(r[2]):<8} | {str(r[3]):<11} | {str(r[4]):<11} | {str(r[5]):<11} | {str(r[6]):<11} | {str(r[7]):<10} | {str(r[8]):<10}")
