...
def main():
    ...
    SELECT secuencia, bastidor, actual_start, actual_end
    FROM #SeqsToSchedule
    WHERE actual_start IS NOT NULL OR secuencia IN ('0261', '0273');
    """
    
    cursor.execute(query)
    rows = cursor.fetchall()
    for r in rows:
        print(f"Seq: {r[0]}, Bastidor: {r[1]}, Start: {r[2]}, End: {r[3]}")
