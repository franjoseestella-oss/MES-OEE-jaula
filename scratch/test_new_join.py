...
LEFT JOIN (
    ...
    FROM dbo.LOG_TABLA
) l ON l.NSECUENCIA = TRY_CAST(m.secuencia AS INT) AND l.rn = 1;

-- Now assign planned times
SELECT 
    s.secuencia AS [Secuencia],
    s.bastidor AS [Bastidor ERP],
    s.log_bastidor AS [Bastidor Log],
    CASE 
...
WHERE 
    s.planned_date = '2026-06-24'
ORDER BY s.slot_idx ASC, s.secuencia ASC;
"""

cursor.execute(query)
rows = cursor.fetchall()
print("Sequence statuses for June 24:")
for r in rows:
    print(f"Secuencia={r[0]}, Bastidor ERP={r[1]}, Bastidor Log={r[2]}, Estado={r[3]}")
