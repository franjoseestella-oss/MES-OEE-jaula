...
-- Simulate Grafana variable $__timeFrom() for June 15, 2026
DECLARE @ActiveDate VARCHAR(8);
SET @ActiveDate = '20260615';
...
WHERE TRY_CAST(erp.fecha_montaje AS DATE) BETWEEN @Monday AND @Friday;

-- First, any sequence that was completed (completed = 1) is scheduled on the day it actually ran
...
BEGIN
    ...
    -- How many slots are already occupied by completed sequences on this day?
    ...
    -- Schedule pending sequences if current date is not in the past relative to today
...
-- Now assign slot_idx, planned_start and planned_end based on scheduled_date
