...
SELECT 
    CONVERT(VARCHAR(19), time, 120) as t_str, 
    metric, 
    value 
FROM (
...
print("REALTIME QUERY TIMELINE:")
for r in cursor.fetchall():
    if r[2] is not None:
        print(r)

print(f"ActiveSlotIdx: {cursor.execute('SELECT @ActiveSlotIdx').fetchone()[0] if hasattr(cursor, 'description') else 'N/A'}")
print(f"TheoreticalActiveSlotIdx: {cursor.execute('SELECT @TheoreticalActiveSlotIdx').fetchone()[0] if hasattr(cursor, 'description') else 'N/A'}")
conn.close()
