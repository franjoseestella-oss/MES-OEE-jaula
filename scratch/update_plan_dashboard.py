...
sql_query = """DECLARE @UTCOffset INT = DATEDIFF(hour, GETUTCDATE(), GETDATE());

-- Get timezone-adjusted range from Grafana time limits
DECLARE @SelectedDateStart DATE = CAST(CAST($__timeFrom() AS datetime2) AT TIME ZONE 'UTC' AT TIME ZONE 'Romance Standard Time' AS DATE);
...
WHERE m.planned_date BETWEEN @SelectedDateStart AND @SelectedDateEnd;
...
if updated:
    ...
    print("Successfully updated Panel 10 query in plan_dashboard.json")
else:
