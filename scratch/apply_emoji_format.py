...
# Revert defaults (no color-text on the whole table)
p9["fieldConfig"]["defaults"]["color"] = {"mode": "thresholds"}
p9["fieldConfig"]["defaults"]["custom"]["cellOptions"] = {"type": "auto"}

p16["fieldConfig"]["defaults"]["color"] = {"mode": "thresholds"}
p16["fieldConfig"]["defaults"]["custom"]["cellOptions"] = {"type": "auto"}

# Define SQL queries with inline colored indicator emojis for the measured columns
sql_select_template = (
    ...
    "  THEN '🟢 ' + CAST(CAST(TIEMPO_ELEVACION_MEDIDO_SINCARGA AS DECIMAL(10,2)) AS VARCHAR) + ' s' "
    "  ELSE '🔴 ' + CAST(CAST(TIEMPO_ELEVACION_MEDIDO_SINCARGA AS DECIMAL(10,2)) AS VARCHAR) + ' s' "
    ...
    "  THEN '🟢 ' + CAST(CAST(TIEMPO_DESCENSO_MEDIDO_SINCARGA AS DECIMAL(10,2)) AS VARCHAR) + ' s' "
    "  ELSE '🔴 ' + CAST(CAST(TIEMPO_DESCENSO_MEDIDO_SINCARGA AS DECIMAL(10,2)) AS VARCHAR) + ' s' "
    ...
    "  THEN '🟢 ' + CAST(CAST(TIEMPO_ELEVACION_MEDIDO_CARGA AS DECIMAL(10,2)) AS VARCHAR) + ' s' "
    "  ELSE '🔴 ' + CAST(CAST(TIEMPO_ELEVACION_MEDIDO_CARGA AS DECIMAL(10,2)) AS VARCHAR) + ' s' "
    "END AS [Elev Carga (s)], "
    ...
    "  THEN '🟢 ' + CAST(CAST(TIEMPO_DESCENSO_MEDIDO_CARGA AS DECIMAL(10,2)) AS VARCHAR) + ' s' "
    "  ELSE '🔴 ' + CAST(CAST(TIEMPO_DESCENSO_MEDIDO_CARGA AS DECIMAL(10,2)) AS VARCHAR) + ' s' "
    "END AS [Desc Carga (s)], "
...
print("Successfully updated queries and reverted cell colors in log_dashboard.json!")
