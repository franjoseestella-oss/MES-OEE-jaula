...
SELECT TOP 1 
    ...
    @ActiveStartDT = DATEADD(hour, -@UTCOffset, TRY_CONVERT(DATETIME, 
        SUBSTRING(FECHA_INICIO_CICLE_UTC, 1, 19) -- wait, let's look at FECHA_INICIO_CICLO
    ))
FROM (
    SELECT NBASTIDOR, 
           CASE WHEN LEN(FECHA_INICIO_CICLO) >= 16 THEN
           ...
           ELSE NULL END AS FECHA_INICIO_CICLE_UTC
    FROM dbo.REFERENCIA_EN_CICLO
) a
WHERE FECHA_INICIO_CICLE_UTC IS NOT NULL;
...
SELECT secuencia, bastidor, actual_start, actual_end, planned_start, planned_end
FROM #SeqsToSchedule
ORDER BY slot_idx;
"""

cursor.execute(query)
print("SeqsToSchedule Details:")
