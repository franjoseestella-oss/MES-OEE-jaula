...
try:
    ...
    # 1. Update BoundedTimestamps filter: (use the late-start fix)
    ...
    # 2. Update early start case (already done in file, but ensure it's here)
    ...
    # 3. Add 'AND s.actual_start IS NULL' to BOTH block conditions:
    query = query.replace("WHEN @ActiveSlotIdx IS NOT NULL AND s.slot_idx > @ActiveSlotIdx THEN",
    ...
    print("\nSimulation of Panel 10 query with completed sequences not hidden:")
    print("-" * 75)
    for r in rows:
        if r[2] is not None:
