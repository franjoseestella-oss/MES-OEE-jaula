...
@router.get("/sequences/{sequence_id}/force-nok", response_class=HTMLResponse)
def force_sequence_nok_by_id_endpoint(
    sequence_id: int,
    request: Request,
    confirm: bool = Query(default=False),
    referer: Optional[str] = Query(default=None),
    ...
    if not confirm:
        ...
        confirm_url = f"/api/v1/sequences/{sequence_id}/force-nok?confirm=true&referer={quote(ref_url)}"
        cancel_url = ref_url
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Confirmar Cambio de Estado a NOK</title>
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Outfit:wght@300;400;600;700&display=swap" rel="stylesheet">
    <style>
...
"""
        ...
        # Ejecutar acción
        success = repo.force_sequence_nok_by_id(db, sequence_id)
        if not success:
            raise HTTPException(
                status_code=404, 
                detail=f"Secuencia con ID {sequence_id} no encontrada en la base de datos."
